...
void    get_perp_and_height(t_ray *ray, t_player *player, t_mlx *mlx)
{
    ...
    ray->wall_dist *= cos(ray->camera_x * M_PI / 180); // Correct fish eye effect
    ray->line_height = (int)(mlx->win_height / ray->wall_dist) * 10; // 10 is a magic number
...
void    raycasting(t_data *data, t_player *player, t_mlx *mlx)
{
    ...
    while (x < mlx->win_width)
    {
        ...
        ray.x = x; // Rest ray.x value init_default_ray
        init_ray(&ray, player);
