...
static void dda_algo(t_data *data, float x2, float y2)
{
    ...
    while (i <= step)
    {
        my_mlx_pixel_put(&data->image, (float)data->player->x_pos, (float)data->player->y_pos, 0xBDEDDF);
        (float)data->player->x_pos = (float)data->player->x_pos + xinc;
    	(float)data->player->y_pos = (float)data->player->y_pos + yinc;
        i++;
    }
}
